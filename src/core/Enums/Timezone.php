...
enum Timezone: string
{
    ...
    /**
     * Converts a UTC date time string to the specified timezone.
     *
     * @param string $utcDateTime The UTC date time string.
     * @param string $format The desired output format (default: 'Y-m-d H:i:s').
     * @return string The date time string in the specified timezone.
     * @throws Exception If the input date time string is invalid.
     */
    public function convertFromUtc(string $utcDateTime, string $format = 'Y-m-d H:i:s'): string
    {
        $dateTime = new DateTime($utcDateTime, new DateTimeZone(self::UTC->value));
        $dateTime->setTimezone(new DateTimeZone($this->value));
        return $dateTime->format($format);
    }

    /**
     * Get the enum case by its string value.
     *
     * @param string $timezoneString
     * @return Timezone|null
     */
    public static function fromString(string $timezoneString): ?self
    {
        foreach (self::cases() as $case) {
            if ($case->value === $timezoneString) {
                return $case;
            }
        }
        return null;
